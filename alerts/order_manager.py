...
Functions:
    ...
    _init_order_tracking    : 첫 실행 시딩 (재시작 중복 알림 방지)
    _handle_timeout         : submitted 3분 타임아웃 처리
    _notify_order           : 개별 주문 알림 전송
    _handle_sell_execution  : 매도 체결 → 포지션 삭제 + 손익 기록
    _handle_buy_execution   : 매수 체결 → 포지션 추가
...
def _resolve_name(ticker: str) -> str:
    ...
    try:
        ...
        return get_all_interests().get(ticker, ticker)
    except Exception:
        return ticker


# ---------------------------------------------------------------------------
# 1. 첫 실행 시딩 (재시작 중복 알림 방지)
# ---------------------------------------------------------------------------

def _init_order_tracking(orders: list[dict], queue: dict) -> None:
    """첫 실행: 기존 체결/실패 주문 시딩 + zombie selling/분할익절 포지션 정리."""
    for order in orders:
...
def _handle_timeout(orders: list[dict]) -> list[dict]:
    """submitted 상태 3분 초과 → failed 전환. 변경 시 디스크 저장.

    Returns:
        갱신된 orders 리스트 (디스크 재로드 시 교체됨).
    """
    _timeout_ids: list[str] = []
    for order in orders:
...
def _notify_order(order: dict, notifier: TelegramNotifier) -> None:
    """체결/실패 주문 텔레그램 알림."""
...
# ---------------------------------------------------------------------------
# 4. 매도 체결 → 포지션 삭제 + 손익 기록
# ---------------------------------------------------------------------------

def _handle_sell_execution(order: dict, name: str) -> bool:
    """매도 체결/실패 처리.

    Returns:
        True이면 분할익절이므로 이후 매수 처리 스킵.
    """
    order_id = order.get("id", "")
    status = order.get("status", "")
    side = order.get("side", "")
    ...
    if side == "sell" and rule_name == "자동청산_분할익절":
        ...
        return True  # 분할 익절은 아래 포지션 삭제 로직으로 가면 안 됨
...
def _handle_buy_execution(order: dict, name: str) -> None:
    """매수 체결 → 포지션 추가, 매수 실패 → _buy_in_progress 정리."""
    order_id = order.get("id", "")
    status = order.get("status", "")
    side = order.get("side", "")
    ticker = order.get("ticker", "")
...
def check_order_status() -> None:
    ...
    if not _st._order_init_done:
        _init_order_tracking(orders, queue)
        _st._order_init_done = True
        return

    # submitted 3분 타임아웃
    orders = _handle_timeout(orders)
    ...
    for order in orders:
        ...
        _notify_order(order, notifier)
        is_partial = _handle_sell_execution(order, name)
        if not is_partial:
            _handle_buy_execution(order, name)
